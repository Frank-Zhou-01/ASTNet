...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['name'].split('-')[0].lower() in ['reds', 'gopro', 'dvd', 'bsd', 'real', 'compress']:
...
class VideoRecurrentSplitClipsTestDataset(VideoTestDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            imgs_lq = read_img_seq(img_paths_lq, data_augment=self.opt['data_augment'])
            imgs_gt = read_img_seq(img_paths_gt, data_augment=self.opt['data_augment'])
