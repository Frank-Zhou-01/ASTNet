...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if rank == 0:
            pbar = tqdm(total=len(dataset), unit='folder', dynamic_ncols=True)
            ...
            if i < num_folders:
                for idx in range(visuals['result'].size(1)):
                    ...
                    if save_img:
                        ...
                        imwrite(result_img, img_path, data_augment=dataset.opt['data_augment'])
    ...
    def test(self, i):
        ...
        if flip_seq:
            ...
            with torch.no_grad():
                ...
                self.lq = torch.squeeze(self.lq, dim=0)
                self.lq = Fun.pad(self.lq, (pad_left, pad_right, pad_top, pad_bottom), mode='reflect')
                self.lq = torch.unsqueeze(self.lq, dim=0)

                # self.lq = self.lq[:, :, :, 232:232 + 256, 512:512 + 256]

                # self.lq = self.lq[:, :, :, 72:72 + 576, 128:128 + 1024]

                self.output = self.net_g(self.lq)
                # features = self.net_g.maps
                # features = np.mean(features, axis=1)
                # np.save(f'/home/frank/project/JIIFNet-main/results/{i}', features)
                #
                self.output = self.output[:, :, :, pad_top:H + pad_top, pad_left:W + pad_left]

                # dense_attn = self.net_g.Transformer.Decoder[2].attn.dense_atten
                # sparse_attn = self.net_g.Transformer.Decoder[2].attn.sparse_attn
                #
                # B, T, C, H, W = self.output.shape
                # num_wins = (H // 8) * (W // 8)
                # dense_attn = dense_attn.view(B, T, num_wins, -1, 64, 64)
