...
class TransformerBlock(nn.Module):
    def __init__(self, channels: int, head: int, win_size) -> None:
        ...
        self.attn2 = SCSA(channels, head)
        self.ffn = DGFFN(channels)
...
class Transformer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.up(x)
        x = rearrange(self.resconv(torch.cat((x, rearrange(res1, 'B T C H W -> (B T) C H W')), dim=1)), '(B T) C H W -> B T C H W', B=B)
        x = self.Decoder(x)
...
class ASTNetL_arch(nn.Module):
    def __init__(self, channels: int, transformer_list: list, head_list: list, win_size: list, fusion_blocks: int, refinement_blocks: int) -> None:
        super().__init__()
        ...
        self.Transformer = Transformer(fusion_channels, transformer_list, head_list, win_size)
        ...
        self.channel_up1 = nn.Conv2d(channels, fusion_channels, kernel_size=1)
        self.channel_down1 = nn.Conv2d(fusion_channels, channels, kernel_size=1)
        self.channel_up2 = nn.Conv2d(channels, fusion_channels, kernel_size=1)
        self.channel_down2 = nn.Conv2d(fusion_channels, channels, kernel_size=1)
